...
export async function summarizeProjectMarkdown(
  ...
  console.log("🚀 | codebaseMarkdown:", codebaseMarkdown);
  let prompt = summarizeProjectPrompt.formatString({
    codebaseMarkdown,
  });
  console.log(
    "🚀 | prompt:",
    summarizeProjectPrompt.formatString({ codebaseMarkdown: "" })
  );
...
export const summarizeProject = async (
  ...
  let codebaseMarkdown = await filesToMarkdown(project.files);
